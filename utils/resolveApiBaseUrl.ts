...
/**
 * Resolves the Kudya API base URL.
...
export function getApiConnectionHint(): string {
  ...
  if (isLocalhostUrl(base) || base.includes('127.0.0.1')) {
    return `Set EXPO_PUBLIC_BASE_API=http://YOUR_PC_IP:${DEV_API_PORT} in KudyaParceiro/.env (same Wi‑Fi), run: python manage.py runserver 0.0.0.0:${DEV_API_PORT}`;
  }
  return `API: ${base}`;
}
